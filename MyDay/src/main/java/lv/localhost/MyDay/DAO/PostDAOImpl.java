...
public class PostDAOImpl extends DAOImpl implements PostDAO {
	...
	public boolean createPost(Post p) throws DBException {
		...
		try {
			...
			PreparedStatement preparedStatement = connection
					.prepareStatement("insert into POSTS (TITLE, AUTHOR, BODY, CREATED ) values (?,?,?,NOW())");
			...
			preparedStatement = connection
					.prepareStatement("SELECT * FROM POSTS ORDER BY POST_ID DESC LIMIT 1");
	...
	public boolean removePost(int postID) throws DBException {
		...
		try {
			...
			System.out
					.println("Exception while execute PostDAOImpl.removePost() ");
			e.printStackTrace();
			throw new DBException(e);
		} finally {
			closeConnection(connection);
	...
	public boolean updatePost(Post p) throws DBException {
		...
		try {
			...
			PreparedStatement preparedStatement = connection
					.prepareStatement("update POSTS set TITLE = ?, AUTHOR = ?, BODY = ?, LAST_MODIFIED = NOW() where POST_ID = ?");
			preparedStatement.setString(1, p.getTitle());
			preparedStatement.setInt(2, p.getAuthorID());
			preparedStatement.setString(3, p.getBody());
			...
			System.out
					.println("Exception while execute PostDAOImpl.updatePost() ");
			e.printStackTrace();
			throw new DBException(e);
		} finally {
			closeConnection(connection);
	...
	public Post findPost(int postID) throws DBException {
		...
		try {
			...
			PreparedStatement preparedStatement = connection
					.prepareStatement("select POST_ID, TITLE, CREATED, AUTHOR, BODY from POSTS where POST_ID = ?");
			preparedStatement.setInt(1, postID);
			...
			System.out
					.println("Exception while execute PostDAOImpl.findPosts() ");
			e.printStackTrace();
			throw new DBException(e);
		} finally {
			closeConnection(connection);
	...
	public List<Post> findLatestPosts() throws DBException {
		...
		try {
			...
			PreparedStatement preparedStatement = connection
					.prepareStatement("SELECT POST_ID, TITLE, CREATED, AUTHOR, BODY FROM POSTS ORDER BY CREATED DESC LIMIT 10");

			ResultSet resultSet = preparedStatement.executeQuery();

			while (resultSet.next()) {
				results.add(new Post(resultSet.getInt(1), resultSet
						.getString(2), resultSet.getDate(3), resultSet
						.getInt(4), resultSet.getString(5)));
			}

		} catch (Throwable e) {
			System.out
					.println("Exception while execute PostDAOImpl.findLatestPosts() ");
			e.printStackTrace();
			throw new DBException(e);
		} finally {
			closeConnection(connection);
	...
	public List<Post> searchPost(String key) throws DBException {
		...
		try {
			...
			System.out
					.println("Exception while execute PostDAOImpl.searchPost() ");
			e.printStackTrace();
			throw new DBException(e);
		} finally {
			closeConnection(connection);
